...
@st.dialog("Confirm enscription")
def ui_enscribe(file, message):
    st.write(f"The file will be written to MOD_{file}")
    if st.button("Confirm"):
        enscribe(file, message)

left_col, right_col = st.columns(2)

file = right_col.file_uploader('Upload a file...')

message = left_col.text_area("Message you want to hide")
if left_col.button("Hide"):
    if file:
        ui_enscribe(file, message)
    else:
        st.warning("Must select a file first")
